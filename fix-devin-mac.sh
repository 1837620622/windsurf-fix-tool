...
clean_devin_local() {
    ...
    echo ""
    echo -e "  ${CYAN}~/Library/Application Support/Devin/${NC}         Devin 应用数据"
...
show_menu() {
    ...
    echo ""
    echo -e "${YELLOW}== Devin Local 清理 ==${NC}"
    echo "  25) 删除 Devin Local 对话数据 (acp-events / History / workspaceStorage)"
    echo ""
    echo "  0) 退出"
    echo ""
    echo -ne ${CYAN}请输入选项 [0-25]: ${NC}
    read -r choice
